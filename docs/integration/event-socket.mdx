...
description: "Control FreeSWITCH and consume its event system over TCP through the inbound and outbound Event Socket modes (the interface fs_cli uses)."
...
<SignalWireCallout
  title="The event socket's model, as a cloud WebSocket"
  href="https://signalwire.com/docs/server-sdks/guides/relay-client"
  page="event-socket"
  linkText="See the RELAY realtime SDK">

RELAY, SignalWire's realtime WebSocket API, is the event socket pattern delivered as a service: events in, commands out, typed SDKs in Python and TypeScript. Your controller keeps the same shape; the socket terminates at the platform instead of port 8021.
