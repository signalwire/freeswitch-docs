...
<SignalWireCallout
  title="You already know this model"
  href="https://signalwire.com/docs/swml"
  page="xml-curl"
  linkText="See SWML served per call">

Serving configuration per request is exactly how SignalWire's hosted platform works: each call can fetch SWML, a JSON/YAML call-control document, from your server. The difference is behind the fetch: the platform runs the engine, so no cluster serves `mod_xml_curl` responses.
