...
<SignalWireCallout
  title="Call logic in Python, without the embedding"
  href="https://signalwire.com/docs/server-sdks/guides/build-ai-agents"
  page="scripting"
  linkText="See the Agents SDK">

SignalWire's Agents SDK runs call logic as ordinary Python or TypeScript services: no `mod_lua` embedding, no interpreter inside the switch. The same SDK builds hosted voice AI agents that follow steps you define in code.
