...
description: "Connect FreeSWITCH to the SignalWire platform with mod_signalwire for inbound and outbound PSTN calling over a persistent WebSocket session."
...
<SignalWireCallout
  title="Beyond mod_signalwire"
  href="https://signalwire.com/docs/platform/freeswitch/add-ai-to-freeswitch"
  page="signalwire-chapter"
  linkText="Add a hosted AI agent to this box">

`mod_signalwire` connects this box to the cloud; the platform behind it runs every capability in this manual as an API on the same engine. Appendix D maps each chapter to its hosted equivalent. A popular first step: point one dialplan extension at a hosted AI agent over SIP.
