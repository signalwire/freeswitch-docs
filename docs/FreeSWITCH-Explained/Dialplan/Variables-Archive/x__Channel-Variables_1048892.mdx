...
This is set to the hangup cause of the last bridged B leg of the call. If you have continue\_on\_fail=true and hangup\_after\_bridge=false you can do checks on this to see what "really" happened to the call. You can for instance do execute\_extension after bridge, do a condition check on ${bridge\_hangup\_cause} to see if it contains MEDIA\_TIMEOUT and then trigger a redial of the call or transfer to a cell phone. For a list of hangup causes, see [Hangup Causes](../../Troubleshooting-Debugging/Hangup-Cause-Code-Table.mdx#about) \[[old wiki](https://wiki.freeswitch.org/wiki/Hangup%5FCauses)\].
...
This is set to the hangup cause of the A leg of the call (note that as such it doesn't make much sense before the end of the call). Often this will take the hangup cause from the B leg of the call, if there is one. For a list of hangup causes, see [Hangup Causes](../../Troubleshooting-Debugging/Hangup-Cause-Code-Table.mdx#about).
...
This is set to the hangup cause of the last bridged B leg of the call. For a list of hangup causes, see [Hangup Causes](../../Troubleshooting-Debugging/Hangup-Cause-Code-Table.mdx#about).
