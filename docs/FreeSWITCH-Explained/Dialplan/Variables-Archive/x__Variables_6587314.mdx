...
When handling transfer/REFER FreeSWITCH normally inherits the dialplan from the original channel. This variable forces the dialplan in which to handle the transfer/REFER
...
Record leg A and leg B streams (i.e. the caller is recorded to the left channel and the receiver is recorded on right channel) into different channel in a stereo file.
