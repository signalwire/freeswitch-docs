...
# Variables Master List 
 ...
 Created by  Ryan Harris, last modified by  livem Chan on 2020.08.25
...
dtmf digit Selects the meta key to use with the [bind\_meta\_app](../Modules/mod_dptools_1970333.mdx#-c) dialplan application.
...
* you are using a [loopback channel](../Bit-Bucket/For-Consideration/mod_bridgecall_13173999.mdx#loopback) to execute a bridge to an endpoint which sends back early media and the received early media's audio is degraded.
...
string Caller ID name to use when dialing endpoints to join the conference via the [conference\_set\_auto\_outcall](../Conference/index.mdx#-freeswitch--conference-) application.
...
string Caller ID number to use when dialing endpoints to join the conference via the [conference\_set\_auto\_outcall](../Conference/index.mdx#-freeswitch--conference-) application.
...
string Conference flags to set for members joining conference via the [conference\_set\_auto\_outcall](../Conference/index.mdx#-freeswitch--conference-) application.
...
integer Maximum time in seconds that the channel that initiated the [conference\_set\_auto\_outcall](../Conference/index.mdx#-freeswitch--conference-) will wait for members to join the conference.
...
string The value of `conference_auto_outcall_prefix` is prepended to each of [conference\_set\_auto\_outcall](../Conference/index.mdx#-freeswitch--conference-) values, of which there can be more than one.
...
string Conference profile to use for members joining the conference via the [conference\_set\_auto\_outcall](../Conference/index.mdx#-freeswitch--conference-) application.
...
integer Originate timeout to use when joining a member to a conference via [conference\_set\_auto\_outcall](../Conference/index.mdx#-freeswitch--conference-).
...
string readonly The result of [play\_and\_detect\_speech](../Modules/mod_dptools_1970333.mdx#-c).
...
string `export_vars` lists variables to be exported to the other leg upon bridge. Unlike [export](../Modules/mod_dptools_1970333.mdx#-c), it only lists the variables to export without actually setting them.
...
* [mod\_dptools: detect\_speech](../Modules/mod_dptools_1970333.mdx#-c)
* [mod\_dptools: play\_and\_detect\_speech](../Modules/mod_dptools_1970333.mdx#-c)
...
boolean If set to true, the leg will only be [mod\_dptools: intercept](../Modules/mod_dptools_1970333.mdx#-c) if the channel is not bridged to anyone.
...
TODO Does this only work with [mod\_dptools: bind\_digit\_action](../Modules/mod_dptools_1970333.mdx#-c)?
...
| tone\_detect\_frequencies | the frequencies to listen for (delimited by + instead of ,). See [tone\_detect](../Modules/mod_dptools_1970333.mdx#-c) |
...
string Set on the inbound leg to control what caller ID number appears in the caller phone's display. Also see [ignore\_diplay\_updates](Variables-Master-List_16352570.mdx#ignore_display_updates) which affect the processing of these variables.
...
boolean If set to `true`, it will [park](../Modules/mod_dptools_1970333.mdx#-c) the call after bridge returns. This is checked before [transfer\_after\_bridge](Variables-Master-List_16352570.mdx#transfer_after_bridge) and [hangup\_after\_bridge](Variables-Master-List_16352570.mdx#transfer_after_bridge).
...
Allows you to set which [DTMF](../Introduction/Glossary_13173966.mdx#dtmf) tones will terminate

* playback (see [mod\_dptools:playback](../Modules/mod_dptools_1970333.mdx#-c) or [mod\_dptools:play\_and\_detect\_speech](../Modules/mod_dptools_1970333.mdx#-c) for example), or
* recording (see `uuid_record` in [mod\_commands](../Modules/mod_commands_1966741.mdx#--) (i.e., the FreeSWITCH API commands), [mod\_dptools:record](../Modules/mod_dptools_1970333.mdx#-c), and [mod\_dptools:record\_session](../Modules/mod_dptools_1970333.mdx#-c)).
...
* `none` \- undefines the terminator digit, meaning no [DTMF](../Introduction/Glossary_13173966.mdx#dtmf) tone will interrupt playback
* `any` \- any [DTMF](../Introduction/Glossary_13173966.mdx#dtmf) will interrupt playback

Digits not specified as terminators will be collected, but won't interrupt playback.

### 2\. Modifiers

* `+` (plus) - includes the following terminator digit along with the other [DTMF](../Introduction/Glossary_13173966.mdx#dtmf) digits in the returned string
...
If [DTMF](../Introduction/Glossary_13173966.mdx#dtmf) 9 is dialed during playback, it will interrupt playback and return the digit 9 along with any other dialed digits; if `*` is dialed it will only interrupt playback without being included in the [DTMF](../Introduction/Glossary_13173966.mdx#dtmf) string returned:
...
See also: [Proxy Media](../../Channel-Variables-Catalog/proxy_media_16353349.mdx#proxy_media)

**Usage**

![(informazioni)](/images/icons/emoticons/information.svg) [Late Negotiation](../Codecs-and-Media/Codec-Negotiation_2883752.mdx#about) () must be enabled in sip profile for this to work in the dialplan.
...
boolean Recording is appended to file. Set `RECORD_APPEND=true` on the channel and all recordings will behave this way to formats which support it (curently [mod\_sndfile](../Modules/mod_sndfile_32178375.mdx#-freeswitch--mod_sndfile-) for WAV, etc.)
...
Where `data` is either the full path (![(attenzione)](/images/icons/emoticons/warning.svg) must be full path!) to an audio file or [teletone generation markup language](../Modules/mod-dptools/mod-dptools:-gentones/TGML_6586573.mdx#lua).
...
![(informazioni)](/images/icons/emoticons/information.svg) Since r15025 local ringback tone is generated only when there is no early media. To force local ringback despite early media use the [ignore\_early\_media](Variables-Master-List_16352570.mdx#ignore_early_media) channel variable.

![(informazioni)](/images/icons/emoticons/information.svg) Ringback will not be sent unless the variable is set _and_ there's a ringing indication from the party that is being called. See [instant\_ringback](Variables-Master-List_16352570.mdx#instant_ringback) if you want to fake ringback even when ringing indication hasn't been received yet.
...
![(attenzione)](/images/icons/emoticons/warning.svg) setting the [rtp\_timer\_name](Variables-Master-List_16352570.mdx#rtp_timer_name) to `none` will create all sorts of undesired side effects with RTCP (packets not sent, RTCP autoadjust failing, etc.).
...
string Modify how the Caller ID will show up in SIP header of the outbound leg. [privacy](../Modules/mod_dptools_1970333.mdx#-c) on the inbound leg controls the remaining contents of RPID and Privacy headers.
...
![(informazioni)](/images/icons/emoticons/information.svg) FreeSWITCH will "do the right thing" and attach an application/sdp type generated by FreeSWITCH (per your settings) for the B leg as it normally would. The other non-SDP MIME parts just pass through.
...
![(attenzione)](/images/icons/emoticons/warning.svg) This should NOT be confused with [sofia\_profile\_name](Variables-Master-List_16352570.mdx#sofia_profile_name), which is the name of the profile for Leg A.
...
![(attenzione)](/images/icons/emoticons/warning.svg) This should NOT be confused with [sip\_profile\_name](Variables-Master-List_16352570.mdx#voicemail_profile_name).
...
![(attenzione)](/images/icons/emoticons/warning.svg) you must also set [failure\_causes](Variables-Master-List_16352570.mdx#failure_causes)
