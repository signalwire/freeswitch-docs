...
FreeSWITCH Advantage API Key Access Tokens (PAT)s are required to access FreeSWITCH Enterprise install packages.  
[How To Create a FreeSWITCH Advantage API Key Access Token](../how-to-create-a-FSA-API-Key-access-token/index.mdx)  
Use that token to install FreeSWITCH Enterprise (do not use SignalWire Personal Access Tokens here as they are for FreeSWITCH Community).
