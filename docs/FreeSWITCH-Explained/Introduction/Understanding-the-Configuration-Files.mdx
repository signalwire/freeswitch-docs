...
* 1 [XML](#xml)
   * 1.1 [Configuration](#configuration)
      * 1.1.1 [switch.conf](#switchconf)
         * 1.1.2 [modules.conf](#configuration-modules)
   * 1.2 [Dialplan](#dialplan)
   * 1.3 [Chatplan](#chatplan)
   * 1.4 [Directory](#directory)
   * 1.5 [Languages](#languages)
* 2 [Pre-Process Directives](#pre-process-directives)
   * 2.1 [include](#include)
   * 2.2 [set](#set)
   * 2.3 [exec](#exec)
   * 2.4 [exec-set](#exec-set)
   * 2.5 [Comment](#comment)
* 3 [Configuration Modules](#configuration-modules)
   * 3.1 [XML Curl](#xml-curl)
...
This section tells FreeSWITCH which modules to load at startup. You can always load other modules from the command line at a later time. For more info see the [module configuration](../Modules/XML-Modules-Configuration_13173211.mdx) page. When FreeSWITCH starts up, it first loads switch.conf, then modules.conf. As each module is loaded, FreeSWITCH will parse and load the configuration element used by that module.
