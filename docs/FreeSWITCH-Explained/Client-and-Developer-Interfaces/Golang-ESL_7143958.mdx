...
# Golang ESL 
 ...
 Created by  Matt Williamson, last modified by  Alex Sibyakin on 2018.03.26

# Introduction

[GoESL](https://github.com/0x19/goesl) is a very simple and straight forward [Go](http://golang.org/) package designed to interact with FreeSWITCH [ESL](Event-Socket-Library/index.mdx#-new-). GoESL supports both client and server. Server is used to bind and listen for incoming FreeSWITCH messages where client is used for sending API commands. GoESL package contains few helpers which can be found in helpers.go so you can easily answer, hangup or send api events.

# Installation

[GoESL](https://github.com/0x19/goesl) is a package designed to be extended. Standard go get will get you going :) Make sure to have [Go](http://golang.org/) properly setup on your box.
...
You can run this code by saving it as tts\_server.go and than running  
...
| go build client.go && ./clientnot worked$ go build client.go && ./clientbash: ./client: No such file or directory ![](/images/icons/contenttypes/comment_16.png) Posted by denis at ago 09, 2016 12:48 |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
