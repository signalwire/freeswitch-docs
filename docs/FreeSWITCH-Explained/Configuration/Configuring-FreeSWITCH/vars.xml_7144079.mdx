...
# vars.xml 
 ...
 Created by  Italo Rossi, last modified by  John Boteler on 2020.09.03
...
This will print out the directory that contains vars.xml and is the top level for the autoload\_configs, dialplan, directory, and sip\_profiles child directories where most of your configuration will be done, if you only use XML files.

### Important Variables

Notable variables defined in vars.xml include:

* **outbound\_caller\_name** \- Name shown in phones when you make outbound calls
* **outbound\_caller\_number** \- Number shown in phones when you make outbound calls
* **domain** \- IP address or DNS name that entries in the user directory register. This is typically FreeSWITCH itself.
* **external\_rtp\_ip** and **external\_sip\_ip** \- Public IP addresses of this FreeSWITCH system. Safest to use an IP address rather than DNS, in case a device can't resolve the domain.

### Commenting

You can not comment out an X-PRE-PROCESS line as the parser looks specifically for that string. Instead, either change the beginning to &lt;YX-PRE-PROCESS... or just delete the line entirely. First, be certain to copy the original file as a backup.

### Variable Types

Related info from the [FreeSwitch FAQ](../../[DEAD/FAQ/index.mdx#sip):

Q: What is the difference between using a ${var} and $${var} in the configuration files?

A: The `${var}` is expanded as it is encountered in the dialplan. The `$${var}` with double dollar signs is used as a pre-processor variable and is expanded when the configuration files are loaded at start time or by the reloadxml fs\_cli command. See vars.xml in the $${conf\_dir} folder for more information.

### Predefined variables

The following variables are set dynamically - calculated if possible by FreeSWITCH™ - and are available to the config as $${variable}.

You can see their calculated value via fs\_cli by executing `eval $${variable}`
...
The -nonat switch eliminates the nat\_xxx variables above since it disables NAT (Network Address Translation) between your private and public IP addresses.
