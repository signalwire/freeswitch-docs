...
This document presents a short tutorial that allows you to start using a FreeSWITCH™ server as a basic PBX. It is part of the minimal FreeSWITCH configuration which is [available at](https://github.com/voxserv/freeswitch%5Fconf%5Fminimal)

[Stanislav Sinyagin](https://freeswitch.org/confluence/display/~ssinyagin) has graciously permitted us to publish this useful example here and we thank him for his work. The original document is [on github at](https://github.com/voxserv/freeswitch%5Fconf%5Fminimal/blob/master/docs/tutorial%5F01%5Fsimple%5Fpbx.md)

Click here to expand Table of Contents

* 1 [Introduction](#introduction)
* 2 [DNS Configuration](#dns-configuration)
* 3 [Dialplan Contexts](#internal-dialplan-context)  
   * 3.1 [Public Dialplan Context](#public-dialplan-context)  
   * 3.2 [Internal Dialplan Context](#internal-dialplan-context)  
   * 3.3 [pstnout Dialplan Context](#pstnout-dialplan-context)
* 4 [Carrier Outbound Gateway](#carrier-outbound-gateway)
* 5 [User Directory](#user-directory)
...
In this example, we use [int.example.net](http://int.example.net) as the domain name for internal SIP client registrations, and [pub.example.net](http://pub.example.net) as the domain name for external peers to connect to our server.

Thus, the SIP clients would use accounts like [701@int.example.net](mailto:701@int.example.net) for registering on our server, and external peers would use a SIP URL like `sip:attendant@[pub.example.net](http://pub.example.net)` to place unauthenticated calls to our server.
...
Most modern SIP clients look up first a NAPTR DNS record in order to find out the SIP service that is serving the domain. Some DNS hosting providers ([godaddy.com](http://godaddy.com), for example) do not allow adding NAPTR records via their DNS editing GUI. A simple solution would be to point an NS record for a sub–domain to some alternative DNS hosting, such as [dns.he.net](http://dns.he.net) or [gandi.net](http://www.gandi.net/) .
...
A NAPTR record should point to one or several SRV DNS records. The standard allows you to put a TCP SRV record with a higher priorty, but not all SIP clients would understand that. FreeSWITCH supports TCP transport for SIP, listening on the same ports as the UDP transport.

By default, FreeSWITCH uses port 5060 for authenticated SIP requests, and port 5080 for non-authenticated ones. Thus, the SRV records for [int.example.net](http://int.example.net) should point to UDP or TCP port 5060, and use port 5080 for [pub.example.net](http://pub.example.net) records.
...
The structure and contents of a dialplan are described in detail in this FreeSWITCH wiki and in the FreeSWITCH book. It is important to understand the two-pass processing workflow, the `continue` attribute in extensions and `break` attribute in conditions. Also you need to understand the meaning of `inline` attribute in the action statements.
...
* [Link](https://github.com/voxserv/)
* [Link](https://github.com/xlab1)
* [Link](https://txlab.wordpress.com/tag/freeswitch/)
* [Link](https://github.com/voxserv/freeswitch%5Fconf%5Fminimal/blob/master/docs/tutorial%5F01%5Fsimple%5Fpbx.md) — Original source of this article by [Stanislav Sinyagin](https://freeswitch.org/confluence/display/~ssinyagin)
