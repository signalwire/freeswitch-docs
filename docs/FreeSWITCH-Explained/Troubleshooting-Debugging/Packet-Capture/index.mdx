...
# Packet Capture 
 ...
 Created by  John Boteler, last modified by  Giovanni Maruzzelli on 2019.02.04
...
* 1 [tcpdump](#tcpdump)  
   * 1.1 [Examples](#examples)  
         * 1.1.1 [Basic Logging](#basic-logging)  
         * 1.1.2 [Capturing Calls For a Specific User](#capturing-calls-for-a-specific-user-)  
         * 1.1.3 [Using Wireshark to Analyze pcap Files](#-using-wireshark-to-analyze-pcap-files-)
* 2 [ngrep](#ngrep)  
...
* 6 [sngrep](#sngrep)  
   * 6.1 [Tips for using sngrep interface](#tips-for-using-sngrep-interface)
* 7 [Truncate a pcap File](#truncate-a-pcap-file)  
   * 7.1 [With The Wireshark GUI](#with-the-wireshark-gui--)  
   * 7.2 [editcap](#editcap)
* 8 [TLS with sharka](#tls-with-sharka--)
* 9 [TLS, real time, with sngrep](#tls-real-time-with-sngrep)
* 10 [](#editcap)
* 11 [](#editcap)
* 12 [](#editcap)
* 13 [Example Analyses](#example-analyses--)  
   * 13.1 [SIP TLS with sharka](#sip-tls-with-sharka--)  
   * 13.2 [SIP TLS on port 5061](#sip-tls-on-port-5061)  
   * 13.3 [RTP events](#rtp-events)  
   * 13.4 [Analyze RTP Quality](#analyze-rtp-quality)  
   * 13.5 [Remote live capture with local wireshark](#remote-live-capture-with-local-wireshark)  
         * 13.5.1 [Windows workstation to remote linux server](#windows-workstation-to-remote-linux-server)
