...
After you performed due diligence to make sure that your configuration is correct and you are confident that you have discovered a bug in FreeSWITCHⓇ, please follow these steps to report a bug on <RepoLink>GitHub</RepoLink>.
...
* Submit a log from your FreeSWITCHⓇ by turning on the following  
   ...
   * ```text title="fs_cli"
     ...
     fsctl send_sighup
     ```
   * (`fsctl send_sighup` starts a new log before running the scenario, then `fsctl send_sighup` again when scenario complete)
* If you had a segmentation fault collect and submit the backtrace from the coredump file.  
   * See [Creating A Backtrace With gdb (Linux/Unix)](Debugging.mdx#advanced-debugging-techniques).  
...
* * `ngrep -qt -W byline port <sip port> -O filename.pcap` **or**  `tshark -S -w filename.pcap port <sip port>`
* Provide a way to filter the call that contains a bug; for instance,  
   * `sip.Call-ID==<Call-ID>`

## Checklist

To make life easier for the developers, please gather this information prior to posting questions to the in the community or submitting an issue on GitHub.
...
* Are you experiencing a segfault? Y/N  
   ...
   * Supply a [backtrace](Debugging.mdx#advanced-debugging-techniques) (bt and bt full)
...
<https://github.com/signalwire/freeswitch/issues> - GitHub trouble ticket tracking system
