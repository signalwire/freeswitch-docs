...
There are several areas in source code that deal with engergy levels, silence threshold, and silence hits (like in `wait_for_silence`, or conference) but we're specifically gonna target "record" area of source code for this modification.
...
now to see this thing in action well create an extension (311 in my case) and be sure to set the `debug_energy_level` channel variable, which will enable the log lines on `fs_cli`:
