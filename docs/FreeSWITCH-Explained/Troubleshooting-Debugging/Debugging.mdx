...
* The [checklist](./Troubleshooting-Debugging/Reporting-Issues-to-GitHub_5046304) for reporting issues **PLEASE READ THIS FIRST**
* **Git** - <https://github.com/signalwire/freeswitch>
* **pastebin** - <https://pastebin.freeswitch.org> (username/password displayed in challenge box)
...
* If it is something that you know worked in the past and doesn't work now see [Using Git Bisect](../Community/Contributing-Code/Tips-For-Using-Git_1048694#git-bisect---tracking-down-breaks-and-bugs-extremely-rapidly-with-git) for helping us track down exactly what commit it broke in. As long as you can use the command line you can test with this method and it will greatly speed up bug fixing.
* If you know what's wrong and are able to change the code to make the fix then see [GitHub Workflow](../Community/Contributing-Code/Tips-For-Using-Git_1048694#github-workflow). A [GitHub Issue](./Reporting-Issues-to-GitHub) that has a patch just needs the code reviewed and tested is about the fastest way to have a ticket resolved.
* Finally when [Reporting A Bug](./Reporting-Issues-to-GitHub) make sure you submit concise repro steps and as much log data as can support the issue. To easily collect and submit log data and sip traces see [`fs_logger.pl`](https://github.com/mitchcapper/FSMisc/blob/master/fs%5Flogger.md). It can collect all the log data, SIP traces, and even submit individual files up to the pastebin. If needed it can obfuscate log data to remove sensitive information prior to posting. `fs_logger.pl` is very easy to use and doesn't have any dependencies other than `fs_cli` on the box. It works on windows and linux and for windows users without perl there is a compiled exe that can be used. To get started with `fs_logger.pl` see the [Helping Others with fs_logger.pl](https://github.com/mitchcapper/FSMisc/blob/master/fs%5Flogger.md#helping-others-debug-with-fs%5Floggerpl) section. It is recommended when filing a ticket to attach the raw pastebin content to the ticket rather than linking so it is preserved (as pastebin entries can and do go away after a certain amount of time).
...
* Windows Git  
   * Use your Git client (like [TortoiseGit](https://tortoisegit.org/)) to update to the latest Git  
...
To put your console in debug mode, press <kbd>F8</kbd> or type `console loglevel 7`.
For more information about the console please see [mod_console](../Modules/mod_console_1048950)  
  
Alternatively you can use the log file to capture specific information. 

:::note

SIP trace messages (see `TPORT_LOG=1` below) will not go to the log files. An easy way to isolate specific debug information with the log files is to use a manual log file rotation. From the FreeSWITCHⓇ CLI:

```text
fsctl send_sighup
```

This will rotate the log files. Reproduce the symptoms you are troubleshooting and then issue a second `fsctl send_sighup` command. You will have a relatively small log file in the log directory, at which point you can copy and paste that into the [pastebin](./Creating-a-freeswitch.log-pastebin). More information about log files can be found [here](../Modules/mod_logfile_1048990.mdx#about).
...
If using GitHub Issues please attach the file as a .txt file rather than referencing pastebin. This helps make sure the reference doesn't get lost because the pastebins timeout.

:::

:::tip

Use this URL format to download a pastebin entry as text:
...
The [#freeswitch](https://signalwire-community.slack.com/channels/freeswitch) channel on Slack is a good place to chat in real-time with FreeSWITCHⓇ developers, power users, and other new users. Please use proper chat room etiquette and be mindful that there are many people from around the world in the chat room. Be patient, even when others are not. Be kind, even when others are not. Don't take anything personally.

### FreeSWITCHⓇ Users Forum

SignalWire maintains a [forum](https://forum.signalwire.community/c/freeswitch) where you can ask questions. Please use a descriptive subject name and include as much detail as reasonably possible in the message body. 
...
If you have more information on a question that you previously posted to the forum then please reply to the thread with the additional information. 

##### Please Be Patient

You've reported a bug or asked a question and haven't heard anything in a few days. You're itching for a fix. It's okay to post a follow up question, but please be polite. Definitely do NOT accuse the developers of ignoring questions, bug reports or feature requests. Their workload is enormous, and much if it is *pro bono*. If you are in desperate need of a new feature then consider [FreeSWITCHⓇ Advantage](https://signalwire.com/products/freeswitch-enterprise) for paid support.

### Calling The FreeSWITCHⓇ Conference

Occasionally there will be times when users will need to speak live instead of on Slack or the forum. There are many ways to call the voice conference and they are listed [here](../Community/ClueCon-Weekly-Conference-call_3965594.mdx#about) on the main page. Also, there are times when people call the voice conference just to hang out, so feel free to call in and see what is happening.
...
FreeSWITCHⓇ uses the [Sofia-SIP](../Configuration/Sofia-SIP-Stack) stack; in many cases **SIP** and **Sofia** are interchangeable. Debugging SIP includes keeping in mind that your voice traffic is going over a data network and therefore devices on the network may affect your VoIP traffic.

Here are some things to keep in mind:

* NAT/Firewalls will cause problems if not [properly configured](../Networking/NAT_6587473)
...
To stop the SIP trace you will need to stop and restart FreeSWITCHⓇ without the `TPORT_LOG` environment variable set. See [Troubleshooting FreeSWITCHⓇ](./Troubleshooting-Freeswitch.mdx) for more information on SIP tracing.
...
Traditional telephony, or Time-Division Multiplexing (TDM), is handled in FreeSWITCHⓇ by the [FreeTDM](./Client-and-Developer-Interfaces/FreeTDM/Freetdm.conf-Examples_13173079) abstraction layer. It is important to provide copies of your FreeTDM configuration files as well as any dialplan entries that use FreeTDM:
...
Run `gcore` on the PID in question to generate a core file. Then run `thread apply all bt` to see what's going on. Most likely you'll need to capture this information and put it into pastebin and then ask one of the advanced FreeSWITCHⓇ users for assistance on Slack. Most likely you'll need to open a GitHub issue, so be ready to do that. Also, see the next section ("example") for some specific steps for collecting information to put into the your issue.
...
The output from the backtrace (`bt`) will hopefully yield the necessary information about why the CPU usage spiked. As always, open an issue with this information. If you wish to discuss the backtrace with other developers you can actually do that on the GitHub issue.

## Advanced Debugging Techniques

### Creating A Backtrace With gdb (Linux/Unix)

If you are getting core dumps (aka Segmentation Faults or "segfaults") in Linux/Unix then it is important to get a backtrace, which is a tool for developers to examine what happened right up to the point where the program crashes. Any report of a segfault should be accompanied by a backtrace. Once you have a backtrace please attach it as a txt file to the relevant <RepoLink>GitHub</RepoLink> issue.

For further information on the very powerful gdb debugger see [Debugging with GDB](https://betterexplained.com/articles/debugging-with-gdb/)
...
```bash
top -H -p `cat /usr/local/freeswitch/run/freeswitch.pid`
```

Find the process that is 100% and note its pid:
```bash
gcore `cat /usr/local/freeswitch/run/freeswitch.pid`
gdb `which freeswitch` <name of resulting file generated by gcore>
```
...
* [Debugging FreeSWITCHⓇ](./Debugging-Freeswitch)
* [Common Errors](./Common-Errors)
* [Submitting Patches](../Community/Contributing-Code/Pull-Requests_3966501)
