...
### Script file swap
Updating LUA files while mod_lua is running usually works fine, but there is a small risk that mod_lua will try to read your LUA script while your text editor saves it, which could result in a half written script getting executed. To eliminate this risk in busy production environments you can use the luaswap command.
Usage:
```
luaswap file.lua
```
The above command will rename `file.lua` to `file.lua.old` and `file.lua.new` to `file.lua`, all while ensuring that mod_lua will not attempt to read the file until the operation is complete.
