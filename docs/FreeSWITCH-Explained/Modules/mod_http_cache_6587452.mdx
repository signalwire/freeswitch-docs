...
For HTTPS support, grab the latest CA certs from [http://curl.haxx.se/ca/cacert.pem](http://curl.haxx.se/ca/cacert.pem) and install in /usr/local/freeswitch/conf. An older copy is also available in freeswitch/src/mod/applications/mod\_http\_cache/conf/cacert.pem.
