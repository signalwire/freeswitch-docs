...
* The script is passed an XML\_REQUEST object which has section, tag\_name, key\_name, and key\_value (see [mod\_xml\_curl](../mod_xml_curl_1049001.mdx)). For some configuration requests, for directory requests, and for dialplan requests, an event object is also passed in the variable 'params'.
