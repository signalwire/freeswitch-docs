
# mod_dptools: gentones 

 

gentones vs tone\_stream

This entry contains [Tone\_stream](./gentones/Tone-stream/Tone_stream_6586599.mdx#about) and [TGML](./gentones/TGML_6586573.mdx#lua), but it is `tone_stream` that is used in every example instead of `gentones` (and `gentones` has only 4 search results on the FreeSWITCH confluence):

* [Freeswitch for new people](../../Miscellaneous/Freeswitch-for-new-people_13173027.mdx#about)
* [XML Dialplan](../../Dialplan/XML-Dialplan/index.mdx#about)

## About

Generate [TGML](./gentones/TGML_6586573.mdx#lua) tones.

Click here to expand Table of Contents

**Error rendering macro 'toc'**

null

### Usage

```xml
gentones <TGML>[|loops]
```

### Examples

```xml
<pre>
<!-- Generate a 500ms beep at 800Hz -->
<action application="gentones" data="%(500,0,800)"/>
<!-- Generate a DTMF string -->
<action application="gentones" data="0800500005"/>
<!-- Generate French ringing tones continuously (loop: -1) -->
<action application="gentones" data="%(1500,3500,350,425)|-1"/>
</pre>
```

  
## See Also

[TGML page](./gentones/TGML_6586573.mdx#lua)

  

