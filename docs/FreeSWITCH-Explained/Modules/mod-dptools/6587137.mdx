
# mod_dptools: start_dtmf_generate 

 Created by  Italo Rossi, last modified by  Attila Gulyas on 2019.12.23

---

Table of Contents (click to expand)

* 1 [0\. About](#-freeswitch--mod_dptools-start_dtmf_generate-)
* 2 [1\. Usage](#-freeswitch--mod_dptools-start_dtmf_generate-)
* 3 [2\. Example](#-freeswitch--mod_dptools-start_dtmf_generate-)
* 4 [3\. Notes](#-freeswitch--mod_dptools-start_dtmf_generate-)
* 5 [4\. See Also](#-freeswitch--mod_dptools-start_dtmf_generate-)

---

## 0\. About

Generate DTMF on current channel.

  
## 1\. Usage

```xml
<action application="start_dtmf_generate"/>
```

## 2\. Example

As an example, placing this in the dialplan prior to bridging a call will allow a phone set to rfc2833 OR info to send DTMF tones inband out to the recipient (IVR) or Auto Attendants. Thus changing the outgoing routing from info or rfc2833 to inband.

TODO [RFC 2833](https://tools.ietf.org/html/rfc2833) is obsoleted by [RFC 4733](https://tools.ietf.org/html/rfc4733).

## 3\. Notes

This option WILL NOT WORK with bridge\_early\_media when used as so:

```xml
<action application="export" data="nolocal:execute_on_media=start_dtmf_generate"/>
<action application="set" data="bridge_early_media=true"/>
<action application="bridge" data="sofia/..."/>
```

## 4\. See Also

* [start\_dtmf](./6587132.mdx#0-about) Turn in-band tones into RFC2833

  

