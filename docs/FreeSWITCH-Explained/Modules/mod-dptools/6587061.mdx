...
| &lt;cause>     | The [hangup cause](../../Troubleshooting-Debugging/Hangup-Cause-Code-Table.mdx#about) with which the call should be terminated.                                                                                                                  | ALLOTTED\_TIMEOUT (the default)                                          |
...
`SWITCH_CAUSE_ALLOTTED_TIMEOUT` is the default [hangup cause](../../Troubleshooting-Debugging/Hangup-Cause-Code-Table.mdx#about) so it could have been omitted as well.
