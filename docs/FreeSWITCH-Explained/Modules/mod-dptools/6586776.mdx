...
# mod_dptools: playback 
 ...
 Created by  Jonatas Oliveira, last modified by  Attila Gulyas on 2020.01.15
...
* 3 [2\. Examples](#2-examples)  
   * 3.1 [2.1 Play a file, and don't allow it to be interrupted](#21-play-a-file-and-dont-allow-it-to-be-interrupted)  
   * 3.2 [2.2 Play file from a specific position](#22-play-file-from-a-specific-position)  
   * 3.3 [2.3 Setting Specific Playback Variables](#23-setting-specific-playback-variables)  
   * 3.4 [Playback from remote server](#playback-from-remote-server)
* 4 [3\. Supported file types](#3-supported-file-types)  
   * 4.1 [3.1 mod\_sndfile](#-freeswitch--mod_dptools-playback-)  
   * 4.2 [3.2 mod\_shout](#-freeswitch--mod_dptools-playback-)  
   * 4.3 [3.3 See also](#33-see-also)

---

## 0\. About

Plays a sound file on the current [channel](../../Introduction/Call-Legs_7143972.mdx#0-about).
...
To stop playback, use the `uuid_break` [API](../../Introduction/FreeSWITCH-API_30867723.mdx#-freeswitch--freeswitch-api-) command. See [mod\_commands](../mod_commands_1966741.mdx#--) for usage.
...
The [playback\_terminators](../../../Channel-Variables-Catalog/playback_terminators_16353264.mdx#0-about) [channel variable](../../Dialplan/Channel-Variables_16352493.mdx#about) is set to `none` to disable stopping the playback on [DTMF](../../Introduction/Glossary_13173966.mdx#dtmf) input. (The default terminator is `*`.)
...
The prefix `{var1=val1,var2=val2}` defines variables that will be sent in [PLAYBACK\_START](../../Introduction/Event-System/Event-List_7143557.mdx#playback_start) and [PLAYBACK\_STOP](../../Introduction/Event-System/Event-List_7143557.mdx#playback_stop) events, thus this is especially handy when building [event socket](../../Introduction/Glossary_13173966.mdx#event-socket) applications. In the events, the variables will be named the same way as the variable names you configured (in this case, `var1` and `var2`).
...
| Can I use playback a audio file 3 times ?Any Example ? Not use for or while in script.Thanks. ![](/images/icons/contenttypes/comment_16.png) Posted by livem at nov 08, 2017 03:54 |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [mod\_dptools: loop\_playback](./16351422.mdx#about) ![](/images/icons/contenttypes/comment_16.png) Posted by livem at gen 17, 2019 06:57                                                 |
