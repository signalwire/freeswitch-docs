...
`mod_avmd` detects the beep; it does not decide whether a person or a machine answered. For that classification, see the commercial [`mod_com_amd`](./mod_com_amd.mdx).

<SignalWireCallout
  title="AMD and beep detection as one method"
  href="https://signalwire.com/docs/swml/reference/calling/detect-machine"
  page="mod-avmd"
  linkText="See detect_machine in SWML">

Hosted SignalWire combines answering-machine and fax detection in the `detect_machine` method of SWML, a JSON/YAML call-control markup. The platform includes the commercial AMD engine (`mod_com_amd`), so hosted detection is the commercial-grade version with no licence to buy and no module to load. Set `detect_message_end: true` and detection stops at the beep or end of greeting, the progressive-dialer case this module serves. Results post to your `status_url` webhook, with `wait` selecting blocking or asynchronous detection.
