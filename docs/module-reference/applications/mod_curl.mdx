...
<SignalWireCallout
  title="HTTP from the call, no module"
  href="https://signalwire.com/docs/swml/reference/calling/request"
  page="mod-curl"
  linkText="See request in SWML">

SWML, SignalWire's JSON/YAML call-control markup, calls HTTP APIs mid-call with the `request` method; hosted AI agents can call them as functions with no server at all. The same idea as `mod_curl`, on the hosted platform.
