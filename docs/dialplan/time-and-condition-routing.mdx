...
description: "Route calls by date and time using condition attributes, enabling business-hours routing, holiday schedules, and other calendar-driven flows."
...
Hosted SignalWire evaluates routing in SWML, a JSON/YAML call-control markup, with `switch` and `cond`. Your server can also return a different document per call, so business-hours logic lives in code you already know how to test.
