...
The applications in this chapter map nearly one-to-one to SWML, SignalWire's JSON/YAML call-control markup: `bridge` becomes `connect`, `playback` becomes `play`, `record_session` becomes `record_call`, and `transfer` keeps its name. The hosted methods run on this same engine, with no dialplan XML behind them.
