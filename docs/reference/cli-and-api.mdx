...
description: "Reference for the FreeSWITCH API command set shared by the fs_cli console and the Event Socket api/bgapi methods."
...
<SignalWireCallout
  title="Every uuid_* command is a REST endpoint"
  href="https://signalwire.com/docs/apis/rest/calls"
  page="cli-and-api"
  linkText="See the Calling API">

`originate` is a POST that creates a call; `uuid_transfer`, `uuid_record`, and `uuid_hold` are updates to a live call resource on SignalWire's REST Calling API. Same live-call control, callable from any language, with no `fs_cli` session on a box.
