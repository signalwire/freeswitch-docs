...
<SignalWireCallout
  ...
  page="recipe-conference-pin"
  linkText="See join_conference in SWML">

On hosted SignalWire the PIN check and the room are a few methods of SWML, a JSON/YAML call-control markup:
...
sections:
  main:
    - answer: {}
    - prompt:
        play: "say:Enter the conference PIN."
        max_digits: 4
    - switch:
        ...
        default:
          - play:
              url: "say:That PIN is not valid. Goodbye."
          - hangup: {}
```

Entry tones, muting, recording, and moderator behavior are parameters on the same method; there is no conference profile XML and no media server to size.
