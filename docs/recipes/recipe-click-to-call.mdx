...
<SignalWireCallout
  ...
  href="https://signalwire.com/docs/apis/rest/calls"
  page="recipe-click-to-call"
  linkText="See the Calling API">

`originate` maps to one POST on SignalWire's REST Calling API: create a call to party A, and hand it a document that connects party B when A answers.

```bash
curl "https://example.signalwire.com/api/calling/calls" \
  ...
  -d '{
    ...
    "params": {
      "from": "+15550100",
      "to": "+15550111",
      "url": "https://example.com/connect-to-b.swml"
    }
  }'
```

The document at `url` runs on the answered leg; a one-method `connect` reaches party B. The click-to-call button becomes an HTTP request from any backend, with no Event Socket session to manage.
