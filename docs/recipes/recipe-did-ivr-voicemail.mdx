...
title: "Chapter 33: Inbound DID to IVR to Voicemail"
...
sidebar_label: "33. Inbound DID to IVR to Voicemail"
---
...
<SignalWireCallout
  ...
  page="recipe-did-ivr-voicemail"
  linkText="Build this in SWML">

The auto-attendant this chapter builds is one document in SWML, SignalWire's JSON/YAML call-control markup, assigned to your number in the dashboard. No profiles, no contexts, no menu XML:
...
sections:
  main:
    ...
    - switch:
        ...
        default:
          - play:
              url: "say:Please leave a message after the tone."
          - record:
              beep: true
```

After a `connect`, the variable `connect_result` reports whether the bridge succeeded, so the no-answer path can fall through to `record` the same way.
