...
<SignalWireCallout
  ...
  linkText="See the Server SDK">

On SignalWire the per-call decision lives in code. The Server SDK's `SWMLService` builds and serves the call-control document on every call, the `mod_xml_curl` pattern, so business hours become ordinary Python:
