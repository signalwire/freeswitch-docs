...
description: "Send and receive fax over G.711 audio and T.38 with mod_spandsp, including T.38 gateway mode and the channel variables that control it."
...
SignalWire sends and receives fax with the `send_fax` and `receive_fax` methods of SWML, a JSON/YAML call-control markup. T.38 negotiation and carrier quirks are the platform's problem; you get a document URL and a status webhook.
