...
On SignalWire, founded by the FreeSWITCH team, an IVR menu is a `prompt` and a `switch` in SWML, a JSON/YAML call-control markup. Or drop the tree entirely: the `ai` method answers with a voice agent that follows steps you define in code. A running FreeSWITCH box can reach the same agent over SIP today.
