...
`join_conference` in SWML, SignalWire's JSON/YAML call-control markup, places a caller into a hosted conference on this same engine. No conference profiles, no media capacity to plan. Video rooms are the same pattern with `join_room`.
