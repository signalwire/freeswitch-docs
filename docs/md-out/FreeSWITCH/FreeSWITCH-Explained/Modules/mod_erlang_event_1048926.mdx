...
The gist is that an [XML Dialplan](../Dialplan/XML-Dialplan/index.mdx#why) extension will invoke the remote Erlang node to handle incoming traffic. Depending on the conditions set up with the extension, this traffic could be an incoming call to a range of destination numbers for example.

### Step 1\. Implement the Erlang code 

Running an Erlang module as a distributed Erlang node, locally or remotely, will handle all traffic that gets directed from an extension in the [FreeSWITCH dialplan](../Dialplan/XML-Dialplan/index.mdx#why).
...
You can answer the call, execute applications, hang up, etc. just as you would in an [XML Dialplan](../Dialplan/XML-Dialplan/index.mdx#why). You can also send commands to FreeSWITCH, just as with inbound mode.
