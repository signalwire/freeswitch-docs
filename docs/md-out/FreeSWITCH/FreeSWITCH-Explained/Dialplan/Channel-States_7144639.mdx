...
# Channel States 
 ...
 Created by  Jonatas Oliveira, last modified by  livem Chan on 2021.05.21
...
| CS\_ROUTING          | 2                                  | In the routing state, FreeSWITCH hunts through the [XML Dialplan](./XML-Dialplan/index.mdx#why). When it finds a condition test that returns true, it builds a to-do list with name–value action pairs, including lists of variables to set and [dialplan applications](../Modules/mod_dptools_1970333.mdx#-c) to execute later (such as "bridge"). During this routing state, none of the applications encountered will be executed, except for a limited number that can be run immediately by including the "inline" attribute. |
