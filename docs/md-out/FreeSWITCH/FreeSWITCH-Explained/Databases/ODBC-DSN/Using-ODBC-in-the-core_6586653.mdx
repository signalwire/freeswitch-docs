...
# Using ODBC in the core 
 ...
 Created by  Gabriel de Medeiros Galvão, last modified by  Italo Rossi on 2017.07.21
...
* 2 [Setup unixODBC](#setup-unixodbc)  
   * 2.1 [Ubuntu](#ubuntu)  
         ...
         * 2.1.2 [Install the unixODBC driver](#install-the-unixodbc-driver)  
         * 2.1.3 [Configure odbc.ini and odbcinst.ini](#configure-odbcini-and-odbcinstini)  
                  * 2.1.3.1 [/etc/odbc.ini for MySQL](#etcodbcini-for-mysql)  
                  * 2.1.3.2 [/etc/odbcinst.ini for MySQL](#etcodbcinstini-for-mysql)  
                  * 2.1.3.3 [/etc/odbc.ini for PostgreSQL](#etcodbcini-for-postgresql)  
                  * 2.1.3.4 [/etc/odbcinst.ini for PostgreSQL](#etcodbcinstini-for-postgresql)  
         * 2.1.4 [Testing your connection](#testing-your-connection)  
         * 2.1.5 [Compile Freeswitch and Set Modules to Use ODBC](#compile-freeswitch-and-set-modules-to-use-odbc)  
   * 2.2 [PostgreSQL 8.4 on Debian Squeeze](#postgresql-84-on-debian-squeeze)  
         * 2.2.1 [Fake libodbcinstq1c2-package with equivs](#fake-libodbcinstq1c2-package-with-equivs)  
         * 2.2.2 [odbc-configuration](#odbc-configuration)  
   * 2.3 [CentOS 5.2](#centos-52)  
   * 2.4 [FreeBSD](#freebsd)
* 3 [Create the FreeSWITCH Database](#create-the-freeswitch-database)  
   * 3.1 [MySQL Client 4.x and MySQL Server 5.x](#mysql-client-4x-and-mysql-server-5x)
...
A known workaround is to add the following parameters to ''odbcinst.ini'' in the respective driver-sections(ANSI, Unicode - see [odbc-configuration](#odbc-configuration) above for a full example):
